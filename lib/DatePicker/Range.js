...
export default class DatePickerRange extends Component {
  ...
  getAvailableRangeFromProps (props = this.props) {
    if (!props.availableRange) {
      return {
        begin: moment('1970-1-1').startOf('day'),
        end: moment('2050-12-31').endOf('day')
      }
    }

    let availableRange = {}
    let begin = props.availableRange.begin == null ? moment('1970-1-1').startOf('day') : moment(props.availableRange.begin).startOf('day')
    let end = props.availableRange.end == null ? moment('2050-12-31').endOf('day') : moment(props.availableRange.end).endOf('day')
    let setted = false
  ...
  setSpecialValue = key => e => {
    ...
    switch (key) {
      case 'last7days':
        newData.begin = moment().subtract(7, 'days')
        break
      case 'last15days':
        newData.begin = moment().subtract(15, 'days')
        break
      case 'last30days':
        newData.begin = moment().subtract(30, 'days')
        break
      case 'all':
        newData.begin = this.state.availableRange.begin
        newData.end = this.state.availableRange.end
        break
    }
  ...
  getOptionsView () {
    const {plainMode, alignPosition, alignDirection} = this.props
    return plainMode ? (
      <div className={this.getClassName('plain')}>
        {this.getCalendarView()}
      </div>
    ) : (
      <Popup visible={this.state.expanded}
        alignElement={this.selectionDom} alignPosition={alignPosition} alignDirection={alignDirection}
  ...
  getCalendarView () {
    ...
    return (
      <div className={this.getClassName('options')}>
        <div className={this.getClassName('quickset')}>
          <a href='#' onClick={this.setSpecialValue('last7days')}>过去7天</a>
          <a href='#' onClick={this.setSpecialValue('last15days')}>过去15天</a>
          <a href='#' onClick={this.setSpecialValue('last30days')}>过去30天</a>
          <a href='#' onClick={this.setSpecialValue('all')}>全部</a>
        </div>
        <DatePicker
