...
export default class File extends Component {
  ...
  static defaultProps = {
    multiple: false,
    buttonIcon: <Icon left name='camera' fill='#9facc7' />,
    buttonText: '选择文件'
  }
    ...
    if (acceptedFiles.length > 0) {
      if (onUpload) {
        onUpload(acceptedFiles[0]).then(file => {
          ...
          this.props.showMessage(typeof e === 'string' ? e : '发生了未知错误，上传失败！请重新上传。')
        })
      } else {
  ...
  render () {
    const {id, mimetypes, maxSize, minSize, buttonIcon, buttonText, disabled} = this.props
