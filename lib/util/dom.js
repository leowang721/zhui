...
export function getRelativeOffset (self, element, relativePosition, alignDirection = 'rb') {
  if (!self || !element) {
    return
  }
  ...
  const v = relativePosition.charAt(1) || 'b'
  const h = relativePosition.charAt(0) || 'l'
  ...
  let hdirection = alignDirection.charAt(0) || 'r'
  let vdirection = alignDirection.charAt(1) || 'b'
  const doc = self.ownerDocument
  const docElem = doc && doc.documentElement
  const bodyRect = docElem.getBoundingClientRect()
  if (vdirection === 't') {
    result.bottom = bodyRect.height - result.top
    delete result.top
  }
  if (hdirection === 'l') {
    // 这时需要右对齐
    result.right = bodyRect.width - result.left
    delete result.left
  }
  return result
}
